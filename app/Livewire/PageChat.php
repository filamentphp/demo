...
class PageChat extends Component implements HasActions, HasSchemas
{
    ...
    public function chatChanged(array $event): void
    {
        ...
        if (($event['operation'] === 'created') && ! $this->isOpen && $message?->body && ($message->user_id !== auth()->id())) {
            Notification::make('page-chat-' . $message->id)
                ->title(($message->user->name ?? 'Someone') . ($message->mentionsUser($this->chatUser->id) ? ' mentioned you' : ' sent a message'))
    ...
    public function openThread(?int $id = null): void
    {
        ...
        $this->reset('body', 'editingId', 'editBody', 'limit');
        $this->form->fill(['body' => '']);
    ...
    public function saveEdit(): void
    {
        ...
        $message = $this->messageQuery()->where('user_id', auth()->id())->whereNotNull('body')->findOrFail($this->editingId);
    ...
    public function render(): View
    {
        ...
        return view('filament.chat.panel', [
            ...
            'thread' => $this->threadId ? $this->messageQuery()->with(['user', 'activity'])->findOrFail($this->threadId) : null,
