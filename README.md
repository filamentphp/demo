...
Open **Inertia Vue**, **Inertia React**, or **Inertia Svelte** in the sidebar. This is an experiment, not a supported Filament API. After adding the loading state, the Chromium integration matrix with lazy notifications enabled passed all 72 cases across all three frameworks, light/dark modes, and SSR/SPA independently enabled and disabled. Another 36 held-request scenarios cover loading, errors, hydration, and navigation. An earlier run caught Livewire's transient navigation progress bar using the invalid `role="bar"`; that separate accessibility defect remains unfixed even though it did not recur in this run. These checks do not cover the pre-hydration input-loss limitation documented below. The historical observations section describes the earlier Vue prototype unless stated otherwise.
...
When SSR is disabled or returns no content, Blade immediately renders a Filament loading indicator and “Loading page…”. The content area stays busy until the framework reports its first mount, independently of deferred props. Successful SSR content stays visible while hydrating. Failed module loads show an error and a **Reload page** action; reloading the document also retries failed module imports without reusing the browser's module cache. The sidebar remains usable throughout loading.
